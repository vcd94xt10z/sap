...
FORM execute_bapi USING is_data TYPE gy_data.
  ...
  ls_headdata-matl_type  = `ZNVL`.
  ls_headdata-basic_view = `X`.
  ls_headdata-mrp_view   = `X`.

  CLEAR ls_clientdata.
  ls_clientdata-base_uom = `AF`.
  ...
  CLEAR ls_plantdata.
  ls_plantdata-plant      = `WRAL`.
  ls_plantdata-pur_group  = `025`.
  ...
  ls_storagelocationdata-plant    = `WRAL`.
  ls_storagelocationdata-stge_loc = `T275`.

  CLEAR ls_storagelocationdatax.
  ls_storagelocationdatax-plant    = `WRAL`.
  ls_storagelocationdatax-stge_loc = `T275`.
